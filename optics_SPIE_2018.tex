...
% do we add ~ 2 paragraphs about Richard's method.
...
The architecture assumed for PICO uses three bands per pixel; each 
...
For each pixel type we orient neighboring pixels at 45\dgr to each other as shown in Figure~\ref{fig:QU}.  
Intro words?? To measure Stokes Q and U we adjust the polarization orientation of neighboring pixels such  oriented at 0 and 90\dgr, sensitive to Stokes Q and $\pm 45$\dgr, sensitive to U. Each wafer is laid out to maximize the following . . . scan  . . .  sky hits . . . language . . .
Q/U ? and syst.

Each MCP is sensitive to only Stokes Q or U. To measure both Q and U we rotate the polarization orientation of adjancent pixels which scan over the same sky location by 45\dgr as shown in 
Figure~\ref{fig:QU}. Adjacent columns of Q and U pixels sample the same sky locations at a slight delay minimize systematics from drifts in detector response which are slower than the scan 
speed. order 0.5 sec is longer than the sample time even at 1 deg beams.

QU observations of single location follow each other on the sky, so same location is observed in Q and U 'simultaneously' almost.
...
photon noise. We assume $\xi=1$. For PICO, NEP$_{Bose}$/NEP$_{Poisson}= 1.5 $ in the lowest band, but since Bose noise does not scale with 
$\sqrt{\nu}$ as Poisson does NEP$_{Bose}$/NEP$_{Poisson} <10\%$ at 321~GHz. % this is NEP/NEP NOT squared.  Squared is 2.2 at 21 GHz and 0.01 at 321. < 10 at 129 GHz.  in NEP to NEP bose = poisson at 30 GHz
%Photon noise dominates at all frequencies for PICO.
