...
coverage. With current best case estimate polarization depth of 0.62~$\mu$K$_{\rm CMB}$-arcmin over the entire 
...
The changing $\eta_{\rm stop}$ has two main effects: changing optical efficiency \como{illumination efficiency?} 
between bands, which affects optical load 
and the conversion of NEP to \como{and therefore the }noise equivalent temperature (NET); and telescope beam size not scaling 
...
with total \como{combined?} noise differing by less than 3\% across all bands.  For both systems \como{you mean readout systems? why 
would the bath temperature depend on the readout system? rephrase?} we require a focal plane temperature, $T_o$, of 
...
and a 0.003\% increase at 799~GHz.  \como{You are not scaling the bose noise down, that's it, right?} 
...
map depth of 0.62~$\mu$K$_{\rm CMB}$-arcmin, which is $\sim$80 times the depth {\it Planck} had achieved. 
