...
For PICO we began \comr{past tense}
the optimization with a Dragone system that to our knowledge has not been implemented \comr{in CMB instruments} before. 
We call \comr{present tense, confusing; choose a tense and stick with it throughtout} it an `open-Dragone' because 
of its overall geometry and in contrast to the widely used `cross-Dragone', see Figure~\ref{fig:ray}. 
...
\comb{We consider} \comr{present tense} \comb{two additional Dragone systems, a Gregorian Dragone and a cross-Dragone, and compare their performance to 
...
area of the open-Dragone,\cite{core2018_inst} the Gregorian Dragone is \comr{present tense} unable to 
support $\mathcal{O}(10^4)$ detectors, so it was \comr{past tense} rejected.  
...
its sidelobes with further reducing the aperture. We therefore concluded that for PICO, an open-Dragone presented
