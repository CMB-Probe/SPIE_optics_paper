...
of view, and that cancel coma, or coma and astigmatism to first order}\cite{dragone}\comr{give more references}.  
...
Adding a stop in this way increases the size of the primary mirror, the primary is unevenly illuminated at various 
field angles. Actively cooling the aperture stop, however, reduces detector noise, and the stop shields the 
...
Dragone's corrections to elimiate coma.\cite{dragone_coma} Dragone's correction calls for a distortion of the primary and secondary 
...
This optimization using only Zernike polynomials increases the diffraction limited area by 15\% at 21~GHz, by 2.4$\times$ at 155~GHz, and by 
...
corrections at the mirror edges are not well constrained.
...
to enforce beam clearences and place an upper limit on overall system size.  Once the optimization coveraged to an acceptable optical 
...
shows that the optimization reduces the overall telescope volume, allowing it to fit more easily within the shadow cone, and increases the DLFOV.  
