...
the transition edge sensor bolometers baselined for PICO required a telecentric focal plane which was sufficiently flat that it 
could be tiled by 10~cm detector wafers without reduction in optical quality. 
...
We found the largest cross-Dragone which met the PICO volume constraints had 
...
\comr{move these to later; let's talk on the phone.} \como{Where else makes sense for this comparison that has few details? I think here or nowhere is most reasonable.}
...
%optical efficiency.  
From these tests we concluded our model was correct and our assumptions were reasonable.
...
accounted for 12\% more power than the reflectors;  \comr{show components in the Figure?} \como{This is not really feasible in the availible time. We can just quote the numbers and remove `see the left . . .'.  Or leave as is.}
