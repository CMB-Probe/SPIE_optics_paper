...
We describe the current state of the PICO instrument design.  We discuss the choice of optical system, present the design of the focal plane, 
...
We consider two additional Dragone systems, a Gregorian Dragone and a cross-Dragone, and compare their performance to 
the open-Dragone. We compare the DLFOV at 155 GHz measured in units of $(F\lambda)^2$ as this metric is invariant to changes 
in $F$-number. 
With half the diffraction limited focal plane 
area of the open-Dragone,\cite{core2018_inst} the Gregorian Dragone is unable to 
support $\mathcal{O}(10^4)$ detectors, so it is rejected.  
...
than a similar open-Dragone, because the cross-Dragone focal length must be long enough that the focal plane does 
not block the primary mirror.
...
These disadvantages with the Gregorian Dragone and the cross-Dragone led us to use the open-Dragone as a starting 
point for the PICO optical design.
...
\comb{We considered two additional Dragone systems, a Gregorian Dragone and a cross-Dragone, and compared the 
relative performance 
of the three systems in terms of DLFOV, compactness, and rejection of sidelobes.  
...
The cross-Dragone had roughly $4\times$ the DLFOV compared to the open-Dragone, 
but was more difficult to pack inside the spacecraft volume {\it and} avoid known sidelobes, as shown in Figure~\ref{fig:sidelobes}.
\comr{why is this Figure 4 and not 3?}
We found that the largest cross-Dragone that met the PICO volume constraints had 
a 1.2~m aperture and an F\# of ? while the largest open-Dragone aperture was 1.4~m and an F\# of ?. The larger 
$F$-number of the cross-Dragone system implied larger physical focal plane size, and therefore higher mass and cost, 
for the same number of pixels. Although the 1.2~m cross-Dragone could accommodate a larger DLFOV, we could not mitigate
its sidelobes without further reducing the aperture. We therefore concluded that for PICO, an open-Dragone presented
...
\comr{the previous paragraph was past tense; choose a tense and stick to it}
...
by \comb{factors of 1.15, 2.4 and 10.5 at 21, 155 and 799~GHz, respectively.} 
%15\% at 21~GHz, by 2.4$\times$ at 155~GHz, and by 10.5$\times$ at 799~GHz. 
\comr{2.4$\times$ is not proper English writing; tell Erin to stick to teaching math ...}  
%2.4$\times$? I don't think so. 2. Why change between \% and $\times$?} \como{1. Yes, according to Erin.} \como{2. Because 1050\% is awkward, as is 1.15$\times$.} 
\comb{We improve the DLFOV at all frequencies by approximately 50\% by rerunning the optimization and including a curved focal surface. A small ($\sim$4\%) additional gain in DLFOV} is achieved by adding Zernike terms 
...
pixel is rotated by 45~deg, Stokes U. This Q/U measurement is in the instrument reference frame with the $x$-axis parallel to the scan direction. 
We rotate neighboring pixels which scan over the same sky location by 45~deg, as shown in Figure~\ref{fig:QU}.
...
where $P_{\rm elem}$ is the in band power emitted by a given element for a single polarization and $\eta_{\rm elem}$ is the efficiency 
of the element. %We assumed 25\% fractional bandwidth top-hat bands and throughput of $\lambda^2$. 
Power emitted by the stop is a special case. We multiply $P_{\rm stop}$ by 
$(1-\eta_{\rm stop})$ because $\eta_{\rm stop}$ is spillover efficiency, the fraction of the throughput which passes through the stop, therefore $(1-\eta_{\rm stop})$ 
...
The jumps in load between neighboring bands, seen in Figure~\ref{fig:popt} around 70 and 200~GHz, are due to $\eta_{\rm stop}$ changing with frequency 
...
The changing $\eta_{\rm stop}$ has three main effects; uneven optical load between bands, varying NEP to noise equivalent temperature (NET) conversion 
...
\hline
Pixel  & Band  & FWHM   & Bolometer NEP & Bolometer NET        & N$_{\rm bolo}$ & Array NET            & Polarization map depth  \\
Type   & GHz   & arcmin & aW/$\sqrt{Hz}$ & $\mu$K$_{\rm CMB}\sqrt{s}$ &           & $\mu$K$_{\rm CMB}\sqrt{s}$ & $\mu$K$_{\rm CMB}$-arcmin      \\ \hline
